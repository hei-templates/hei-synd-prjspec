...
set-version version:
  ...
  find {{project_dir}} -name "*.typ" -exec sed -i 's/@preview\/hei-synd-prjspec:[0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*/@preview\/hei-synd-prjspec:{{version}}/g' {} \;
  echo "Done"

# update hei-synd-prjspec import version in all .typ files
[macos]
set-version version:
  #!/usr/bin/env sh
  echo "Updating hei-synd-prjspec version to {{version}}"
  find {{project_dir}} -name "*.typ" -exec sed -i '' 's/@preview\/hei-synd-prjspec:[0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*/@preview\/hei-synd-prjspec:{{version}}/g' {} \;
  echo "Done"
